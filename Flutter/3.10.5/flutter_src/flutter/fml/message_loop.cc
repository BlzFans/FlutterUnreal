...
template <typename T>
class ThreadLocalUniquePtr2 : public ThreadLocalUniquePtr<T> {
 public:
  ~ThreadLocalUniquePtr2() {
    if (!ownership) {
#if FML_THREAD_LOCAL_PTHREADS
      this->ptr_.swap(nullptr);
#else
      this->ptr_.release();
#endif
    }
...
FML_THREAD_LOCAL ThreadLocalUniquePtr2<MessageLoop> tls_message_loop;
